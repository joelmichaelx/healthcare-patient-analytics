#!/usr/bin/env python3
"""
Healthcare Patient Analytics Platform - Large Dataset Population Script
Creates a comprehensive healthcare dataset with 100,000+ records from 2020 to present
"""
...
def create_large_patient_dataset(num_patients=1000):
    """Create a large dataset of patients with realistic demographics"""
    ...
    conditions = [
        ...
        "Arthritis", "Asthma", "Depression", "Anxiety", "Obesity",
        "Alzheimer's", "Parkinson's", "Epilepsy", "Migraine", "Fibromyalgia"
    ]
    
    risk_levels = ["Low", "Medium", "High", "Critical"]
    risk_weights = [0.4, 0.3, 0.2, 0.1]  # More low/medium risk patients
    ...
    for i in range(num_patients):
        ...
        # Generate admission date between 2020 and now
...
def create_large_vital_signs_dataset(patients_df, records_per_patient=100):
    """Create large vital signs dataset"""
    print(f"Creating vital signs records...")
    ...
    for patient_id in patient_ids:
        ...
        # Generate vital signs over time
        start_date = datetime(2020, 1, 1)
        end_date = datetime.now()
...
def create_large_lab_results_dataset(patients_df, records_per_patient=50):
    """Create large lab results dataset"""
    print(f"Creating lab results records...")
    ...
    lab_tests = [
        ...
        {"name": "Cholesterol", "unit": "mg/dL", "range": "<200", "critical_low": 100, "critical_high": 400},
        {"name": "Triglycerides", "unit": "mg/dL", "range": "<150", "critical_low": 50, "critical_high": 600},
        {"name": "BUN", "unit": "mg/dL", "range": "7-20", "critical_low": 3, "critical_high": 50},
        {"name": "Albumin", "unit": "g/dL", "range": "3.5-5.0", "critical_low": 2.0, "critical_high": 6.0}
    ]
...
def create_large_medications_dataset(patients_df, records_per_patient=200):
    """Create large medications dataset"""
    print(f"Creating medication records...")
    ...
    medications_list = [
        ...
        {"name": "Amlodipine", "dosage": 5, "unit": "mg", "route": "Oral"},
        {"name": "Metoprolol", "dosage": 50, "unit": "mg", "route": "Oral"},
        {"name": "Omeprazole", "dosage": 20, "unit": "mg", "route": "Oral"},
        {"name": "Prednisone", "dosage": 10, "unit": "mg", "route": "Oral"},
        {"name": "Lorazepam", "dosage": 1, "unit": "mg", "route": "Oral"}
    ]
    ...
    for patient_id in patient_ids:
        ...
        if condition == "Diabetes":
            ...
            relevant_meds = ["Lisinopril", "Amlodipine", "Metoprolol"]
        elif condition == "Heart Disease":
            relevant_meds = ["Aspirin", "Atorvastatin", "Metoprolol", "Digoxin"]
        elif condition == "COPD":
            relevant_meds = ["Albuterol", "Prednisone"]
        else:
            relevant_meds = random.sample([med["name"] for med in medications_list], random.randint(2, 5))
        ...
        for med_name in relevant_meds:
            ...
            for i in range(records_per_patient // len(relevant_meds)):
                medications.append({
                    ...
                    "administered_by": f"Nurse_{random.randint(1, 50):03d}",
...
def populate_large_database():
    ...
    print("Healthcare Patient Analytics Platform - Large Dataset Population")
    print("=" * 60)
    ...
    print("\nCreating large healthcare dataset...")
    
    # 1. Create 1000 patients
    patients_df = create_large_patient_dataset(1000)
    
    # 2. Create vital signs (100 records per patient = 100,000 records)
    vital_signs_df = create_large_vital_signs_dataset(patients_df, 100)
    
    # 3. Create lab results (50 records per patient = 50,000 records)
    lab_results_df = create_large_lab_results_dataset(patients_df, 50)
    
    # 4. Create medications (200 records per patient = 200,000 records)
    medications_df = create_large_medications_dataset(patients_df, 200)
    ...
    # Summary
    print("\n Large Dataset Population Complete!")
    print("=" * 40)
    ...
    print(f"   • Time Range: 2020 to {datetime.now().strftime('%Y-%m-%d')}")
    print(f"\n Your healthcare analytics platform now has enterprise-scale data!")
    print(f"   Dashboard ready for production use with realistic healthcare data")
